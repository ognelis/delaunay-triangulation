...
class Triangle(var a: Vertex, var b: Vertex, var c:Vertex) extends Drawable {
  ...
  def edgeWithOtherTriangle(thatTriangle: Triangle): Unit = {
    for {
      thisEdge <- this.edges
      thatEdge <- thatTriangle.edges if thisEdge == thatEdge
    } yield {
...
//  }
  ...
  final def findTrinagleForThatPoint(vertex: Vertex): Triangle = {
    ...
    if ((crossProduct(a, first.a, first.b) >= 0.0 ^ crossProduct(vertex, first.a, first.b) >= 0.0) && first.triangle2 != null)
      first.triangle2.findTrinagleForThatPoint(vertex)
    else if ((crossProduct(b, second.a, second.b) >= 0.0 ^ crossProduct(vertex, second.a, second.b) >= 0.0) && second.triangle2 != null)
      second.triangle2.findTrinagleForThatPoint(vertex)
    else if ((crossProduct(c, third.a, third.b)  >= 0.0 ^ crossProduct(vertex, third.a, third.b) >= 0.0) && third.triangle2 != null)
      third.triangle2.findTrinagleForThatPoint(vertex)
    else this
  }
