...
import delone.{ConvexHull, Delone, Vertex}
    ...
    def helper(points: List[Vertex], acc: List[Vertex]): List[Vertex] = points match {
      ...
      case head :: tail => acc match {
        case last :: preLast :: stack => if (crossProduct(head, last, preLast) >= 0)
          helper(points, acc.tail) else helper(tail, head :: acc)
        case _ => helper(tail, head :: acc)
      }
    }
    if (points.length == 2) points
    else points match {case head :: next :: tail => helper(tail, next :: head :: Nil)}
  }
...
//      pixels foreach (pixel => canva.pixelDrawing.fillOval(pixel.x,pixel.y,5,5))
  ...
  val computeButton = new Button("compute") {
    onMouseClicked = (e: MouseEvent) => {
      val delone =  new Delone(ConvexHull((canva.points ++= randomPoint.pixels).distinct))
